...
contract("FidelToken", async accounts => {
    it("Staking 100x2", async () => {
        ...
        await fidelToken.mint(accounts[1], 1000);
        // Get init balance of user
        balance = await fidelToken.balanceOf(owner)

        // Stake the amount, notice the FROM parameter which specifes what the msg.sender address will be

        stakeID = await fidelToken.stake(stake_amount, { from: owner });
    ...
    it("new stakeholder should have increased index", async () => {
        let stake_amount = 100;
        stakeID = await fidelToken.stake(stake_amount, { from: accounts[1] });
        // Assert on the emittedevent using truffleassert
        // This will capture the event and inside the event callback we can use assert on the values returned
        truffleAssert.eventEmitted(
            stakeID,
    ...
    it("cannot stake more than owning", async () => {
        ...
        try {
            await fidelToken.stake(1000000000, { from: accounts[2] });
        } catch (error) {
            assert.equal(error.reason, "FidelToken: Cannot stake more than you own");
        }
    });
    ...
    it("cant withdraw bigger amount than current stake", async() => {
        ...
        try {
            await fidelToken.withdrawStake(200, 0, {from:owner});
        }catch(error){
    ...
    it("withdraw 50 from a stake", async() => {
        ...
        await fidelToken.withdrawStake(withdraw_amount, 0, {from:owner});
        // Grab a new summary to see if the total amount has changed
        let summary = await fidelToken.hasStake(owner);
    ...
    it("calculate rewards", async() => {
        ...
        await fidelToken.stake(1000, {from: owner});
        await helper.advanceTimeAndBlock(3600*20);

        summary = await fidelToken.hasStake(owner);
    ...
    it("reward stakes", async() => {
        ...
        await fidelToken.mint(accounts[3],1000);
        let initial_balance = await fidelToken.balanceOf(staker);
        // Make a stake on 200, fast forward 20 hours, claim reward, amount should be Initial balanace +4
        await fidelToken.stake(200, {from: staker});
        await helper.advanceTimeAndBlock(3600*20);

        let stakeSummary = await fidelToken.hasStake(staker);
        let stake = stakeSummary.stakes[0];
        // Withdraw 100 from stake at index 0 
        await fidelToken.withdrawStake(100, 0, { from: staker});

        // Balance of account holder should be updated by 104 tokens
        let after_balance = await fidelToken.balanceOf(staker);
        ...
        let second_balance = await fidelToken.balanceOf(staker);
